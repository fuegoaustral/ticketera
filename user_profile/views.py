...
def download_ticket_pdf(request, ticket_key):
    ...
    from tickets.models import NewTicket
    from tickets.ticket_pdf import build_new_ticket_pdf_bytes

    try:
        ticket = NewTicket.objects.select_related(
            'event', 'ticket_type', 'owner', 'holder', 'owner__profile'
        ).get(key=ticket_key)
    except NewTicket.DoesNotExist:
    ...
    if ticket.event.attendee_must_be_registered and not ticket.owner:
        return HttpResponse(
            'Este bono no puede ser descargado porque no está vinculado a una cuenta. Por favor, vincúlalo primero.',
            status=403,
        )

    try:
        pdf_bytes = build_new_ticket_pdf_bytes(ticket)
    except ImportError:
        return HttpResponse(
