...
class TicketAdmin(admin.ModelAdmin):
    ...
    list_filter = ('order__order_tickets__ticket_type__event', 'order__status', 'volunteer_ranger', 'volunteer_umpalumpa', 'volunteer_transmutator', )
    search_fields = ('first_name', 'last_name', 'key', )
    actions = ['export', 'resend_email', ]
    readonly_fields = ('key', )

    def get_queryset(self, request):
        return super(TicketAdmin, self).get_queryset(request).select_related('order__order_tickets__ticket_type__event')
    ...
    @admin.display(ordering='order__order_tickets__ticket_type__event__event_id', description='Event')
    def get_event(self, obj):
        return obj.order.order_tickets.first().ticket_type.event
...
class TicketTypeAdmin(admin.ModelAdmin):
    ...
    def get_queryset(self, *args, **kwargs):
        ...
        qs = qs.annotate(tickets_sold=Count('order_tickets__order', filter=Q(order_tickets__order__status=Order.OrderStatus.CONFIRMED)))
        return qs
...
class OrderAdmin(admin.ModelAdmin):
    list_display = ('id', 'status', 'get_event', 'first_name', 'last_name', 'ticket_count', 'amount')
    list_filter = ('order_tickets__ticket_type__event', 'status', 'order_tickets__ticket_type')
    search_fields = ('first_name', 'last_name', 'order_tickets__ticket_type__name', 'order_tickets__ticket_type__event__name')
    inlines = [TicketInline, ]
    actions = ['export']

    def get_queryset(self, request):
        return super(OrderAdmin, self).get_queryset(request).select_related('order_tickets__ticket_type__event')

    @admin.display(ordering='order_tickets__ticket_type__event__event_id', description='Event')
    def get_event(self, obj):
        return obj.order_tickets.first().ticket_type.event
    ...
    def export(self, request, queryset):
        ...
        for order in queryset:
            writer.writerow([
                ...
                order.amount,
                order.coupon,
                order.order_tickets.first().ticket_type.name if order.order_tickets.exists() else '',  # Assuming first ticket type
                order.order_tickets.aggregate(total_tickets=Count('quantity'))['total_tickets']
            ])

        return response

    def ticket_count(self, order):
        return order.order_tickets.aggregate(total_tickets=Count('quantity'))['total_tickets']
...
class CouponAdmin(admin.ModelAdmin):
    list_display = ('token', 'get_ticket_type', 'get_event', 'max_tickets', 'tickets_remaining', 'display_url', )
    ...
    def get_ticket_type(self, obj):
        return obj.order.order_tickets.first().ticket_type if obj.order.order_tickets.exists() else None
...
class TicketTransferAdmin(admin.ModelAdmin):
    ...
    list_filter = ('ticket__order__order_tickets__ticket_type__event', 'transferred', )
    list_display = ('first_name', 'last_name', 'ticket', 'get_event', 'transferred', )

    def get_queryset(self, request):
        return super(TicketTransferAdmin, self).get_queryset(request).select_related('ticket__order__order_tickets__ticket_type__event')

    @admin.display(ordering='ticket__order__order_tickets__ticket_type__event__event_id', description='Event')
    def get_event(self, obj):
        return obj.ticket.order.order_tickets.first().ticket_type.event
