...
Incluye `ONLINE_PURCHASE`, transferencias internacional/local, `CASH_ONSITE`, `MP_QR_CAJA`, `MP_POINT_CAJA`, `OTHER`. El default es compra online.

Las ventas **caja v2** crean órdenes con el `OrderType` según método de pago ([`caja/services/sales.py`](../caja/services/sales.py)); `generated_by_admin_user` apunta al operador que vendió.

## Ventas caja v2

1. `CajaSale` inicia en `PENDING`; al confirmar pago pasa a `PAID` vía `finalize_caja_sale`.
2. Si hay líneas con `EventProduct` vinculado a `TicketType`, se crea `Order` **CONFIRMED**, `OrderTicket` y `NewTicket` (mismas reglas de owner que caja legacy).
3. Efectivo y transferencia finalizan al instante; MP QR/Point esperan confirmación de MercadoPago Instore (`external_reference = caja-sale-{id}`).
4. Productos genéricos (sin bono) suman ingreso en reportes pero no generan `Order`.

Ver [caja-v2](caja-v2.md).
