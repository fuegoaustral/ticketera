...
| **Acceso caja (Mi Fuego)** (`Event.access_caja`) | Admin del `Event` o flujos en Mi Fuego que agregan/quitan usuarios | Aparece en **`/mi-fuego/cajas/`**, puede vender en **caja v2** y usar caja legacy si aplica ([`caja.permissions`](../caja/permissions.py)). |
...
### Tres flujos de “caja” (no son lo mismo)
...
2. **Caja Mi Fuego legacy (operadores del evento)** — **`/mi-fuego/cajas/`** → **`/mi-fuego/mis-eventos/<slug>/caja/`**  
   - Requiere: usuario logueado y ser **`admins`** del evento **o** estar en **`access_caja`** para ese evento.  
   - Solo tipos con **`show_in_caja=True`** ([`CajaEmitirBonoForm`](../user_profile/forms.py)); respeta reglas de emisión y opción “marcar como usada (venta en puerta)”.  
   - **GET** en esa URL: si el evento tiene **exactamente una** caja v2 activa (`EventCaja`), redirige a la caja v2; si no, al listado de cajas.

3. **Caja v2 (módulo `caja`)** — **`/mi-fuego/mis-eventos/<slug>/cajas-v2/<id>/vender/`**  
   - Requiere: **`admins`** o **`access_caja`** para vender; solo **`admins`** para productos, configuración MP y reportes.  
   - Productos configurables (bonos vinculados a `TicketType` o ítems genéricos), stock unificado, múltiples cajas por evento.  
   - Métodos de pago: efectivo, transferencia, **Mercado Pago QR** y **Postnet** (Instore).  
   - Documentación completa: [caja-v2](caja-v2.md).

Un operador puede tener **access_caja** sin ser staff: usa la caja de Mi Fuego (v2 o legacy), no `/admin/caja/`.
...
- Configuran **productos**, **cajas v2**, **stock**, **MercadoPago Instore** y ven **reportes** del evento (ventas caja, reporte consolidado, stock). Ver [caja-v2](caja-v2.md).
...
| Formulario Mi Fuego caja legacy | [`user_profile/views.py`](../user_profile/views.py) (`caja_view`), [`user_profile/forms.py`](../user_profile/forms.py) (`CajaEmitirBonoForm`) |
| Caja v2 (productos, ventas, reportes) | [`caja/`](../caja/), [caja-v2](caja-v2.md) |
